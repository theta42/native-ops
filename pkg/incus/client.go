...
func (c *Client) GetContainerIP(ctx context.Context, name string) (string, error) {
	...
	for time.Now().Before(deadline) {
		...
		_, _ = c.exec.Run(ctx, fmt.Sprintf("incus exec %s -- rm -f /etc/resolv.conf || true", name))
		resolvB64 := base64.StdEncoding.EncodeToString([]byte("nameserver 1.1.1.1\nnameserver 8.8.8.8\n"))
		_, _ = c.exec.Run(ctx, fmt.Sprintf("echo '%s' | base64 -d | incus file push - %s/etc/resolv.conf || true", resolvB64, name))
